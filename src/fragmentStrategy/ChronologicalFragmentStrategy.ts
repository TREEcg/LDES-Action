 ...
 */
class ChronologicalFragmentStrategy implements IFragmentStrategy {

    fragment(data: RDF.Quad[][], config: IConfig): void {

        let sortedData: RDF.Quad[][] = this.sort(data, 'http://www.w3.org/ns/prov#generatedAtTime');
    ...
    sort(data: RDF.Quad[][], predicate: string): RDF.Quad[][] {
        return data.sort((a, b) => {
            let identifierA = date.dateToBasicISODate(new Date(this.find(a, predicate)));
            let identifierB = date.dateToBasicISODate(new Date(this.find(b, predicate)));
...
export default ChronologicalFragmentStrategy;