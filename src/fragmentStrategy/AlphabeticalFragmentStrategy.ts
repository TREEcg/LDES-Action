...
class AlphabeticalFragmentStrategy implements IFragmentStrategy {
		...
		pages.forEach((page, index) => {
			...
			let nextPage =
				index + 1 < pages.length
					? this.getFileLocation(index + 1, config)
					: null;
			// previous page
			let previousPage =
				index > 0 ? this.getFileLocation(index - 1, config) : null;
	...
	createHypermedia(
		...
		treeMembers.forEach((member) => {
			hypermedia.push(
				quad(
					namedNode('https://smartdata.dev-vlaanderen.be/base/gemeente'),
		...
		hypermedia.push(
			quad(
				...
				namedNode(`${config.gh_pages_url}/${fileLocation}`)
			)
		);
		...
		if (nextPage) {
			...
			hypermedia.push(
				quad(
					...
					namedNode(`${config.gh_pages_url}/${nextPage}`)
				)
			);
		}
		if (previousPage) {
			...
			hypermedia.push(
				quad(
					blankNode('Relation_Previous'),
					namedNode('https://w3id.org/tree#node'),
					namedNode(`${config.gh_pages_url}/${previousPage}`)
				)
			);
		}
